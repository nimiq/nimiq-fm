...
export function runningAway(digits: number[], _batch: number, _blockNumber: number) {
  ...
  const basslead = note('[g1|g1|g#1|d2|g2|g#2|d3|g3]*8')
    ...
    .rel(0)
    .gain(0.7)
    .when('[0 [0|1]]*4', (x: any) => x.lpf(600).lpenv(6).lpd(0.22).rarely((x: any) => x.clip(0.7).ply('2|2|2|2|3')))
    .superimpose((x: any) => x.s('supersaw').trans(24).mul(gain(0.9)))

  const kick = note('g1*4')
    .sound('sbd')
    .dist(0.4)
    .gain(0.5)
    .sometimesBy(0.5, (y: any) => y.when('< 0 [0!3 1]>', (x: any) => x.clip(0.75).ply('2|2|2|4')))
    .orbit(2)
    .duckorbit(1)
  ...
  return stack(
    ...
    crash,
    top,
  ).reset('<1!7 [1!2 [[1]|[1!2]] [[1!2]|[1!2]|[1!4]]]>').when('<0!7 [0!3 1]>', (x: any) => x.clip(0.5)
    .someCyclesBy(0.2, (x: any) => x.trans(saw.fast(4).range(0, 4)).speed(saw.fast(4).range(1, 1.3))))
}
