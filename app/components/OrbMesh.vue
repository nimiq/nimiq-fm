...
onBeforeRender(({ delta, elapsed }) => {
    ...
    if (!graphData.value || !nodesMeshRef.value || !linesRef.value) return
    ...
    if (groupRef.value) groupRef.value.rotation.y += delta * 0.02
    ...
    for (let i = 0; i < nodes.length; i++) {
        ...
        if (blockFlash > 0) {
            ...
            // Standard State
            if (n.baseColor) tempColor.copy(n.baseColor)
            if (n.type === NodeType.VALIDATOR) {
