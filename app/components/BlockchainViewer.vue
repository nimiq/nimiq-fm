...
const addBlock = (blockNumber: number, type: 'micro' | 'macro' = 'micro', validatorAddress?: string) => {
...
<template>
  <div class="w-full overflow-hidden py-4">
        ...
        >
          ...
          >
            <div class="text-center w-full">
              <div class="text-[0.5rem] font-bold text-white/90 uppercase tracking-wider mb-0.5">MACRO</div>
              <div class="text-[0.55rem] font-bold text-white font-['Orbitron'] leading-none tracking-tighter">M{{ block.blockNumber.toString().slice(-4) }}</div>
            </div>
            ...
            <div class="text-center w-full">
              <div class="text-[0.6rem] font-bold text-white font-['Rajdhani']">{{ block.votes }} / 512</div>
            </div>
          </div>
          ...
          >
             <div class="w-full">
                <div class="text-[0.5rem] font-bold text-white font-['Orbitron'] leading-none tracking-tighter">#{{ block.blockNumber }}</div>
             </div>
             ...
             <div class="w-full">
                <div class="text-[0.55rem] text-slate-500 font-mono">Slot {{ block.slot }}</div>
                 <div class="flex gap-0.5 mt-1 opacity-30">
                   <div class="w-0.5 h-0.5 rounded-full bg-slate-400" v-for="n in 3" :key="n"></div>
                 </div>
             </div>
          </div>
        </Motion>
    ...
    <div class="mt-2 px-8 flex items-center justify-end gap-1.5 opacity-10">
       <div v-for="n in 30" :key="n" class="w-1 h-1 rounded-sm" :class="n % 6 === 0 ? 'bg-blue-500 w-2 h-2 -my-0.5' : 'bg-slate-500'"></div>
    </div>
  </div>
</template>
